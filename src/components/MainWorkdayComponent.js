...
class MainWorkdayComponent extends React.Component {
  ...
  async workdayIsValid(workdayObj) { // Called after user adds a new workday to punchcard
    let resultObj = {}
    await axios.post(`${window.location.origin}/checkforoverlap`, // Check if there is a workday in the database that overlaps with this newly-added workday
      {
        params: {
          workday: workdayObj
        }
  ...
  addWorkdaysToDb = () => {
    ...
    axios.post(`${window.location.origin}/addworkdays`, // Check if there is a workday in the database that overlaps with this newly-added workday
    {
      params: {
